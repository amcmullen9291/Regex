...
export default function Landing() {
  ...
  const definitionsHTMLCharClass = ["placeholder" , "Matches the first occurance of either term 'x' or term 'y'.", "A wildcard; this matches any single character. However, inside square brackets, it matches a literal dot or period.", "", "This expression will match any digit. It is equivalent to the Character Case expression [0-9].", "This expression matches any character that is not a digit. It is equivalent to Character Case expression [^0-9].", "", "This expression looks for the character used to signify the end of a line of text and the start of a new line which is ' \\n '.", "This expression looks for the control character used to reset a cursor's position to the beginning of a line of text (like when you use the return key).", "Match a single white space character.", "Match a single character that is NOT white space. ","This expression matches a horizontal tab.", "", "Matches any alphanumeric character from the basic Latin alphabet, including the underscore. It is equivalent to the expression [A-Za-z0-9_].", "Matches any character that is not a word character. It is equivalent to [^A-Za-z0-9_].","","A negated or complemented character class. That is, it matches any value NOT equal to whatever character or range is enclosed in the brackets.", "[abcd] is the same as [a-d]. This expression matches any one of the enclosed characters. [abcd-] matches any of the enclosed characters as well as the underscore.A negated or complemented character class matches anything that does not match what is enclosed in the brackets."];
  ...
  const definitionsQuantifiers = [ "placeholder" , "The asterisk will look for the preceding character 0 or more times. Note: if multiple letters are searched for, ALL of the letters will have to appear in the searched string in the same order as the entered regex filter value. The asterisk only affects the character immediately before it.","Matches the preceding value 1 or more times. It is equivalent to the expression {1,}.","A question mark ( ? ) immediately following a character will match zero or more instances of that value. If added after another quantifier it switches the matching mode from greedy to lazy. That means: 'repeat the minimal number of times needed'.","Matches exactly 'n' occurrences of the preceding given value. ","Matches at LEAST 'n' occurrences of the preceding value.","Matches at least 'n' and at most 'm' occurrences of the preceding value. Both 'n' and 'm' have to be positive and and m > n."];

  const HTMLCharClasses = [ < span key={"2"} onClick={(e) => {define(e, "2")}}>. </span>, <span>, </span>, < span key={"4"} onClick={(e) => {define(e, "4")}}>\d </span>,<span>, </span>, < span key={"5"} onClick={(e) => {define(e, "5")}}>\D </span>,<span>, </span>, < span key={"7"} onClick={(e) => {define(e, "7")}}>\n </span>,<span>, </span>, < span key={"8"} onClick={(e) => {define(e, "8")}}>\r </span>,<span>, </span>, < span key={"9"} onClick={(e) => {define(e, "9")}}>\s </span>,<span>, </span>, < span key={"10"} onClick={(e) => {define(e, "10")}}>\S </span>,<span>, </span>, < span key={"11"} onClick={(e) => {define(e, "11")}}>\t </span>,<span>, </span>, < span key={"13"} onClick={(e) => {define(e, "13")}}>\w </span>,<span>, </span>, < span key={"14"} onClick={(e) => {define(e, "14")}}>\W </span>,<span>, </span>, < span key={"16"} onClick={(e) => {define(e, "16")}}>[^  ]</span>, <span>,</span>,< span key={"17"} onClick={(e) => {define(e, "17")}}>[x-y]</span> , <span>, </span>, < span key={"1"} onClick={(e) => {define(e, "1")}}>[x|y]</span>]
  ...
  const HTMLQuantifiers = [< span key={"1"} onClick={(e) => {define3(e, "1")}}>*</span>, <span>, </span>,< span key={"2"} onClick={(e) => {define3(e, "2")}}>+</span>, <span>, </span>,< span key={"3"} onClick={(e) => {define3(e, "3")}}>?</span>, <span>, </span>,< span key={"4"} onClick={(e) => {define3(e, "4")}}>x&#x007B;n&#x007D;</span>, <span>, </span>,< span key={"5"} onClick={(e) => {define3(e, "5")}}>x&#x007B;n,_&#x007D;</span>, <span>, </span>,< span key={"6"} onClick={(e) => {define3(e, "6")}}>x&#x007B;n,m&#x007D;</span> ];
...
function lookForRepeat(e){
  e.preventDefault();
    ...
    setEndOfQuerySlash("\\ig");
    document.getElementById("bottomDiv").innerHTML = "The 'g' modifier specifies a global match. It will find all matches present in a string. By default quantifiers are 'greedy', meaning that they try to match as much of the string as possible.";
  }else{
